...
grading but that you may find useful in verifying that your code works. We also encourage you to develop your own simpledb.test
...
To run the unit tests use the `simpledb.test` build target:

```
$ cd [project-directory]
$ # run all unit tests
$ ant simpledb.test
$ # run a specific unit simpledb.test
...
If you wish to write new unit tests as you code, they should be added to the <tt>simpledb.test/simpledb</tt> directory.
...
ant simpledb.test|Compile and run all the unit tests.
ant runtest -Dtest=testname|Run the unit simpledb.test named `testname`.
ant systemtest|Compile and run all the system tests.
ant runsystest -Dtest=testname|Compile and run the system simpledb.test named `testname`.
...
JUnit tests that live in the <tt>simpledb.test/simpledb/systemtest</tt> directory. To run all the system tests, use
...
<p>This indicates that this simpledb.test failed, showing the stack trace where the error was detected. To debug, start by reading the source code where the error occurred. When the tests pass, you will see something like the following:
...
It is likely you'll want to create your own tests and your own data tables to simpledb.test your own implementation of SimpleDB.
...
* Click finish, and you should be able to see "simple-db-hw" as a new project in the Project Explorer tab on the
  ...
  code can be found in "src," and unit tests and system tests found in "simpledb.test."
...
you don't switch, you may see spurious simpledb.test failures in some of the performance tests in later labs.

**Running Individual Unit and System Tests**

To run a unit simpledb.test or system simpledb.test (both are JUnit tests, and can be initialized the same way), go to the Package
Explorer tab on the left side of your screen. Under the "simple-db-hw" project, open the "simpledb.test" directory. Unit tests
are found in the "simpledb" package, and system tests are found in the "simpledb.systemtests" package. To run one of
these tests, select the simpledb.test (they are all called *Test.java - don't select TestUtil.java or SystemTestUtil.java), right
click on it, select "Run As," and select "JUnit Test."  This will bring up a JUnit tab, which will tell you the status
of the individual tests within the JUnit simpledb.test suite, and will show you exceptions and other errors that will help you
debug problems.

**Running Ant Build Targets**

If you want to run commands such as "ant simpledb.test" or "ant systemtest," right click on build.xml in the Package Explorer.
...
want to run (probably "dist" and one of "simpledb.test" or "systemtest"). This should run the build targets and show you the
...
* At this point, you should be able to pass the ScanTest system simpledb.test, which is the goal for this lab.
...
interfaces of your code because you will be implementing transactions and locking in a future lab. The simpledb.test code we have
...
<p>At this point, you should be able to complete the ScanTest system simpledb.test. Good work!
...
public class simpledb.test {
    ...
    public static void main(String[] argv) {
        ...
        Database.getCatalog().addTable(table1, "simpledb.test");
...
appropriate table (in the case of `SeqScan`) or child operator (in the case of e.g. Filter). The simpledb.test program then
...
We **strongly recommend** you try this out as a fun end-to-end simpledb.test that will help you get experience writing your own
simpledb.test programs for simpledb. You should create the file "simpledb.test.java" in the src/java/simpledb directory with the code above, 
...
```
ant
java -classpath dist/simpledb.jar simpledb.simpledb.test
```

Note that `ant` compiles `simpledb.test.java` and generates a new jarfile that contains it.
...
* A .java file we can drop in the simpledb.test/simpledb directory, compile, and run.
...
**Important:** before testing, gradescope will replace your <tt>build.xml</tt> and the entire contents of the <tt>simpledb.test</tt>
directory with our version of these files. This means you cannot change the format of <tt>.dat</tt> files!  You should
also be careful changing our APIs. You should simpledb.test that your code compiles the unmodified tests.
