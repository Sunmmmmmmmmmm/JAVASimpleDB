...
discuss options).  Additionally, we are providing extra source and simpledb.test files
...
the simpledb.test for the ant targets `simpledb.test` and
...
Furthermore, you should be able to pass the AggregateTest system simpledb.test.
...
methods do no evict pages from the buffer pool to properly pass this simpledb.test.
...
remove a page from the buffer pool *without* flushing it to disk. We will not simpledb.test `discardPage()`
in this lab, but it will be necessary for future labs.

At this point, your code should pass the EvictionTest system simpledb.test.

Since we will not be checking for any particular eviction policy, this simpledb.test works by creating a BufferPool with 16
...
eviction policy correctly, you will not evict enough pages, and will go over the size limitation, thus failing the simpledb.test.
...
need to load this only for the purposes of this simpledb.test).
...
SeqScan) or child operator (in the case of e.g., Join). The simpledb.test program then repeatedly calls `next` on the `Join`
...
* A <tt>.java</tt> file we can drop in the
  `simpledb.test/simpledb` directory, compile, and run.
...
entire contents of the <tt>simpledb.test</tt>
