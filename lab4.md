...
discuss options).  Additionally, we are providing extra simpledb.test cases
...
implemented this code in lab 2, but we did not simpledb.test for this case.)
...
internal error or deadlock has occurred.  The simpledb.test cases we have provided
...
At this point, your code should pass the `TransactionTest` unit simpledb.test and the
`AbortEvictionTest` system simpledb.test.  You may find the `TransactionTest` system simpledb.test
...
`simpledb.test/simpledb/DeadlockTest.java`. They are actually a
...
the simpledb.test checks if locks have been acquired and the waiting time before
...
Your code should now should pass the `TransactionTest` system simpledb.test (which
...
* Given that this lab will require you to heavily modify your earlier code, regression testing passing is a prerequisite
  for grading tests. This means that if your submission fails a simpledb.test from earlier labs, you will get a 0 for the
