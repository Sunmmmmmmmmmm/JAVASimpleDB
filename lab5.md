...
options).  Additionally, we are providing extra source and simpledb.test files for this
...
`BTreeChecker.java`. This method is also used to simpledb.test your B+Tree implementation
...
in `systemtest/BTreeFileInsertTest.java`.  Some of the system simpledb.test cases may
take a few seconds to complete. These files will simpledb.test that your code inserts
...
in `BTreeDeadlockTest.java` and `BTreeInsertTest.java`. Some of the simpledb.test cases
may take a few seconds to complete. `BTreeDeadlockTest` will simpledb.test that you have
implemented locking correctly and can handle deadlocks. `BTreeInsertTest` will
simpledb.test that your code inserts tuples and splits pages correcty, and also handles
...
complete since they create a large B+ tree in order to fully simpledb.test the system.
...
`simpledb.test/simpledb/BTreeDeadlockTest.java` if you have implemented locking correctly
inside of your B+ tree code.

If everything is implemented correctly, you should also be able to pass the
BTreeTest system simpledb.test. We expect many people to find `BTreeTest` difficult, so
it's not required, but we'll give extra credit to anyone who can run it
successfully. Please note that this simpledb.test may take up to a minute to complete.
...
find useful.  You should also write code to simpledb.test that your implementation
