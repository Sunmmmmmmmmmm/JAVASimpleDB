...
public class LockingTest extends TestUtil.CreateHeapFile {
  ...
  /**
   * Generic unit simpledb.test structure for BufferPool.getPage() assuming locking.
   *
  ...
  /**
   * Generic unit simpledb.test structure to grab an additional lock in a new
   * thread.
   *
  ...
  public void grabLock(TransactionId tid, PageId pid, Permissions perm,
    ...
    // TODO(ghuo): yes, stop() is evil, but this is unit simpledb.test cleanup
    t.stop();
  }

  /**
   * Unit simpledb.test for BufferPool.getPage() assuming locking.
  ...
  /**
   * Unit simpledb.test for BufferPool.getPage() and BufferPool.releasePage()
   * assuming locking.
   * Acquires read locks on different pages.
   */
